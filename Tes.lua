--[[
    Gabungan Script: Translator + Teleport + AutoDrive Pro + ESP + AIM HACK
    Dibuat oleh: OpenAI GPT & Analis AI
    Kontributor: Yusuf (ESP, Aimbot, Silent Aim Integration)
    Versi: Final Profesional 2.2.0 (6-in-1, Silent Aim Added, Code Cleanup)
    Deskripsi: Mengimplementasikan UI multi-fitur dengan Translator, Teleport, AutoDrive, ESP, dan AIM HACK (Camera & Silent).
               Semua fitur diatur melalui satu GUI dan disimpan dalam satu file konfigurasi.
               [ADD] Menambahkan fitur Silent Aim yang terintegrasi dengan GUI dan pengaturan yang sudah ada.
               ...
               [CLEANUP] Kode diformat ulang untuk keterbacaan yang lebih baik.
    Perubahan: Mengganti header default ImGui dengan header kustom yang bisa minimize.
]]--

script_name("Menu Gabungan Pro")
script_author("DEMON")
...
local minimized = new.bool(false)
local active_menu_item = new.int(0) -- 0=Translator, 1=Teleport, 2=AutoDrive, 3=ESP, 4=AIM HACK
...
local ini_defaults = {
    ...
    aimhack_settings = {
        ...
        maxDistance = 150.0,
        maxFOV = 1.0,
        selectedBone = 1, -- 0=Head, 1=Chest, 2=Groin
        showFOV = true
    }
}
...
local ap = {
    active = {
        autopilot_on = new.bool(false),
        status_window = new.bool(false)
    },
    settings = {
...
-- AIM HACK State
local aimhack = {
    camera_enabled = new.bool(ini.aimhack_settings.camera_enabled),
    silent_enabled = new.bool(ini.aimhack_settings.silent_enabled),
    maxDistance = new.float(ini.aimhack_settings.maxDistance),
    maxFOV = new.float(ini.aimhack_settings.maxFOV),
    selectedBoneInt = new.int(ini.aimhack_settings.selectedBone),
    showFOV = new.bool(ini.aimhack_settings.showFOV)
}
local WIDGET_FIRE = 1
...
function main()
    ...
    sampAddChatMessage("[Menu Gabungan] Ketik {00FF00}/menu{FFFFFF} untuk membuka GUI 6-in-1.", 0xFFFFFF)
    
    lua_thread.create(teleport_loop)
    lua_thread.create(renderInfoBar)
    lua_thread.create(camera_aimbot_loop)
end
...
imgui.OnFrame(function() return show_ui[0] end, function(player)
    ...
    if minimized[0] then
        ...
        imgui.BeginChild("##header_min", imgui.ImVec2(-1, 0), false, imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse + imgui.WindowFlags.AlwaysAutoResize)
        ...
        if imgui.Button("+##restore", imgui.ImVec2(btn_w, btn_h)) then
            minimized[0] = false
        end
    ...
    if not minimized[0] then
        ...
        if imgui.Selectable("AIM HACK", active_menu_item[0] == 4) then active_menu_item[0] = 4 end
        imgui.EndChild()
        ...
        if active_menu_item[0] == 0 then -- Konten Translator
            ...
            if imgui.InputText("##Lang", input_lang, 8) then
                target_lang = u8:decode(input_lang)
            end
            imgui.PopItemWidth()
        ...
        elseif active_menu_item[0] == 1 then -- Konten Teleport
            ...
            if imgui.Button("Teleport ke Marker", btn_size) then
                lua_thread.create(function() startTeleport('') end)
            end
            if imgui.Button("Teleport ke Checkpoint", btn_size) then
                lua_thread.create(function() startTeleport('chp') end)
            end
            ...
            if imgui.Button(bar_text, imgui.ImVec2(-1, 0)) then
                ini.teleport_settings.RenderBar_State = not ini.teleport_settings.RenderBar_State
            end
        ...
        elseif active_menu_item[0] == 2 then -- Konten AutoDrive Pro
            ...
            if imgui.Button("Menuju Waypoint", imgui.ImVec2(button_width, 40)) then
                getTargetBlipCoordinatesFixed(function(s, x, y, z)
                    if s then
                        ap.status.target = {x = x, y = y, z = z}
                        startAutoPilot('waypoint')
                    else
            ...
            imgui.SameLine()
            if imgui.Button("Hentikan Autopilot", imgui.ImVec2(button_width, 40)) then
                stopAutoPilot("~y~Dihentikan manual.")
            end
            ...
            if imgui.Button("Ikuti Rute Checkpoint", imgui.ImVec2(-1, 40)) then
                if checkpointCoords.active then
                    startAutoPilot('master')
                else
                    sampAddChatMessage("{FF0000}[AutoDrive]: Tidak ada checkpoint untuk memulai rute.", -1)
                end
            end
            
            imgui.Separator(); imgui.Text(u8"Kecepatan Maksimal:");
            imgui.PushItemWidth(-1);
            if imgui.SliderFloat("##Speed", ap.settings.speed, 10.0, 150.0, "%.0f") then
                ini.autodrive_settings.ap_speed = ap.settings.speed[0]
            end
            imgui.PopItemWidth()
        ...
        elseif active_menu_item[0] == 3 then -- KONTEN ESP
            ...
            imgui.Separator()
            if imgui.Checkbox("Rainbow Color", esp.useRainbow) then
                ini.esp_settings.useRainbow = esp.useRainbow[0]
            end
            if not esp.useRainbow[0] then
                if imgui.ColorEdit4("ESP Color", esp.colorFloat) then
                    ini.esp_settings.colorR = esp.colorFloat[0]
                    ini.esp_settings.colorG = esp.colorFloat[1]
                    ini.esp_settings.colorB = esp.colorFloat[2]
                    ini.esp_settings.colorA = esp.colorFloat[3]
                end
            end

        elseif active_menu_item[0] == 4 then -- KONTEN AIM HACK
            imgui.Text(u8"Pengaturan AIM HACK"); imgui.Separator()
            
            imgui.Text("Mode:")
            if imgui.Checkbox("Camera Aimbot (Tahan Tembak)", aimhack.camera_enabled) then ini.aimhack_settings.camera_enabled = aimhack.camera_enabled[0] end
            if imgui.Checkbox("Silent Aim (Otomatis)", aimhack.silent_enabled) then ini.aimhack_settings.silent_enabled = aimhack.silent_enabled[0] end
            if imgui.Checkbox("Tampilkan Lingkaran FOV", aimhack.showFOV) then ini.aimhack_settings.showFOV = aimhack.showFOV[0] end
            
            imgui.Separator()
            imgui.Text("Pengaturan Umum (berlaku untuk semua mode):")
            imgui.PushItemWidth(-1)
            if imgui.SliderFloat("Jarak Target Maks", aimhack.maxDistance, 10.0, 300.0, "%.0f m") then ini.aimhack_settings.maxDistance = aimhack.maxDistance[0] end
            if imgui.SliderFloat("Field of View (FOV)", aimhack.maxFOV, 0.1, 2.5, "%.2f rad") then ini.aimhack_settings.maxFOV = aimhack.maxFOV[0] end
            imgui.PopItemWidth()
            
            imgui.Separator()
            imgui.Text("Target Tulang (Bone):")
            if imgui.RadioButtonIntPtr("Kepala", aimhack.selectedBoneInt, 0) then ini.aimhack_settings.selectedBone = 0 end; imgui.SameLine()
            if imgui.RadioButtonIntPtr("Dada", aimhack.selectedBoneInt, 1) then ini.aimhack_settings.selectedBone = 1 end; imgui.SameLine()
            if imgui.RadioButtonIntPtr("Perut", aimhack.selectedBoneInt, 2) then ini.aimhack_settings.selectedBone = 2 end
        end
        imgui.EndChild()
...
-- ESP & FOV Drawing Loop
imgui.OnFrame(function() return true end, function()
    local drawList = imgui.GetBackgroundDrawList()
    
    -- Gambar Lingkaran FOV
    if (aimhack.camera_enabled[0] or aimhack.silent_enabled[0]) and aimhack.showFOV[0] then
        local sx, sy = getScreenResolution()
        local crossX, crossY = sx * 0.52, sy * 0.43
        local radius = (sx / 3) * (aimhack.maxFOV[0] / 2.5)
        ...
        lua_thread.create(function()
            ...
            if res and code == 200 then
                ...
                if translated and translated ~= text then
                    ...
                    if #translator_log > 50 then
                        table.remove(translator_log, 1)
                    end
                end
            end
        end)
...
function sampev.onSendBulletSync(data)
    if not aimhack.silent_enabled[0] then
        return true -- Biarkan peluru lewat jika silent aim non-aktif
    end
    ...
    if targetId and targetPos then
        ...
        data.target = { x = targetPos.x, y = targetPos.y, z = targetPos.z }
    end
...
function startTeleport(mode)
    ...
    if mode == 'chp' then
        ...
        found, tX, tY, tZ = getCheckPoint()
    else
        sampAddChatMessage("Mencari Marker...", 0xFFFF00FF)
        found, tX, tY, tZ = getMapMarker()
    end

    if found then
        cx, cy, cz = getCharCoordinates(PLAYER_PED)
        bx, by, bz = tX, tY, tZ
        if not (type(cx) == 'number' and type(bx) == 'number') then
            sampAddChatMessage("Error: Gagal mendapatkan koordinat yang valid.", 0xFF0000FF)
            return
        end
        
        distance = getDistanceBetweenCoords3d(cx, cy, cz, bx, by, bz)
        teleport, teleport_state = true, true
        one_percent = distance > 0 and distance / 100 or 0
        teleport_timer = os.clock()
        sampAddChatMessage("Teleport dimulai ke tujuan!", 0x00FF00FF)
    else
        if mode == 'chp' then
            sampAddChatMessage("Checkpoint tidak ditemukan!", 0xFF0000FF)
        else
            sampAddChatMessage("Marker tidak ditemukan!", 0xFF0000FF)
        end
    end
end
...
function onfoot_teleport_step()
    ...
    for i = 1, math.floor(65 / 2) do
        if distance >= 20 then
            local vec = vector3d(bx - cx, by - cy, bz - cz)
            vec:normalize()
            cx, cy, cz = cx + vec.x * 2, cy + vec.y * 2, cz + vec.z * 2
            send_onfoot_sync(cx, cy, cz, height)
            distance = getDistanceBetweenCoords3d(cx, cy, cz, bx, by, bz)
            if math.random(0, 5) == 5 then
                wait(ini.teleport_settings.Sleep_Onfoot)
            end
        else
            wait(500)
            send_onfoot_sync(bx, by, bz, height)
            setCharCoordinates(PLAYER_PED, bx, by, bz)
            teleport, teleport_state = false, false
...
function incar_teleport_step()
    ...
    for i = 1, math.floor(150 / 5) do
        if distance >= 20 then
            ...
            send_incar_sync(cx, cy, cz, height)
            distance = getDistanceBetweenCoords3d(cx, cy, cz, bx, by, bz)
            if math.random(0, 5) == 5 then
                wait(ini.teleport_settings.Sleep_Incar)
            end     
        else
            wait(1000)
            teleport = false
            send_incar_sync(bx, by, bz, height)
            teleport_state = false
            setCharCoordinates(1, bx, by, bz)
            break
        end
    end
end
...
function send_onfoot_sync(x, y, z, height)
    ...
    data.moveSpeed = { 0.7, 0.7, height and -0.7 or 0.7 }
...
function getEspColor()
    if esp.useRainbow[0] then
        ...
        if rainbowHue > 1.0 then rainbowHue = 0.0 end
...
function drawEspBar(drawList, x, y, w, h, value, color, label)
    ...
    if label and label ~= "" then
        ...
        local textX = x + (w / 2) - (textSize.x / 2)
        local textY = y + h + 2
...
function drawEspSkeleton(drawList, ped, color)
    for _, conn in ipairs(skeleton_connections) do
        local x1, y1, z1 = getBonePosition(ped, conn[1])
        local x2, y2, z2 = getBonePosition(ped, conn[2])
        local r1, sx1, sy1 = convert3DCoordsToScreenEx(x1, y1, z1)
        local r2, sx2, sy2 = convert3DCoordsToScreenEx(x2, y2, z2)
        if r1 and r2 then
            drawEspLine(drawList, sx1, sy1, sx2, sy2, color, 2.0)
        end
    end
end

--[[ ================================================= ]]--
--[[                 FUNGSI-FUNGSI AIM HACK            ]]--
--[[ ================================================= ]]--
...
function convertCartesianToSpherical(src, dst)
    ...
    if phi > 0 then phi = phi - math.pi else phi = phi + math.pi end
...
function getDynamicCrosshairOffset(dist)
    local baseX, baseY = 0.53, 0.45
    local offsetX, offsetY
    if dist <= 10.0 then
        offsetX = baseX - 0.003
        offsetY = baseY - 0.004
    else
        local factor = math.min(dist / 300.0, 1.0)
        offsetX = baseX - (0.01 * factor)
        offsetY = baseY - (0.015 * factor)
    end
...
function getCrosshairSpherical(distance)
    local sx, sy = getScreenResolution()
    local offsetX, offsetY = getDynamicCrosshairOffset(distance)
...
function aimToTarget(tx, ty, tz)
    local px, py, pz = getCharCoordinates(PLAYER_PED)
    local from = vector3d(px, py, pz)
    local to = vector3d(tx, ty, tz)
    ...
    local phiFinal = phiCam + (phiTarget - phiCross)
    local thetaFinal = thetaCam + (thetaTarget - thetaCross)
    
    setCameraRotation(phiFinal, thetaFinal)
end

function getSelectedBone()
    local boneChoice = aimhack.selectedBoneInt[0]
    if boneChoice == 0 then return BONE.HEAD end
    if boneChoice == 2 then return BONE.GROIN end
    return BONE.CHEST -- Default
end
...
function getBestTargetInFOV()
    local nearestDist = 9999
    local bestTarget = { id = nil, ped = nil, pos = nil }

    local px, py, pz = getCharCoordinates(PLAYER_PED)
    local phiCam, _ = getCameraRotation()
        ...
        if sampIsPlayerConnected(i) then
            ...
            if ok and doesCharExist(ped) and not isCharDead(ped) then
                ...
                if x then
                    ...
                    if dist <= aimhack.maxDistance[0] then
                        ...
                        if delta <= aimhack.maxFOV[0] and dist < nearestDist then
                            nearestDist = dist
                            bestTarget.id = i
                            bestTarget.ped = ped
                            bestTarget.pos = { x = x, y = y, z = z }
                        end
                    end
                end
            end
    ...
    return bestTarget.id, bestTarget.ped, bestTarget.pos
end

function camera_aimbot_loop()
    while true do
        wait(0)
        if aimhack.camera_enabled[0] and isWidgetPressed(WIDGET_FIRE) then
            local _, _, targetPos = getBestTargetInFOV()
            if targetPos then
                aimToTarget(targetPos.x, targetPos.y, targetPos.z)
            end
        end
    end
end
...
function getCheckPoint()
    if checkpointCoords.active then
        return true, checkpointCoords.x, checkpointCoords.y, checkpointCoords.z
    end
    return false, 0, 0, 0
end
...
function renderInfoBar()
    while true do
        ...
        if ini.teleport_settings.RenderBar_State and teleport_state then
            local r, g, b, a = rainbow(0.5, 250, 1)
            local resX, resY = getScreenResolution()
            local x, y = resX / 2, resY / 1.5
            
            if teleport_state and one_percent > 0 then
                local time = os.clock() - teleport_timer
                local percent = 100 - math.floor(distance / one_percent)
...
function sampev.onSendPlayerSync()
    if teleport_state then return false end
end
